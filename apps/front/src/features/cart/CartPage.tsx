...
export function CartPage() {
  const user = useAuthStore((state) => state.user)!; const companyId = useCompanyStore((state) => state.storefrontCompanyId); const client = useQueryClient(); const [offline, setOffline] = useState(!navigator.onLine); const [confirm, setConfirm] = useState<ConfirmDialogState | null>(null);
  const cartCacheKey = `${user.id}:${companyId ?? "none"}`;
  const cart = useQuery({ queryKey: ["cart", user.id, companyId], enabled: Boolean(companyId), queryFn: async () => {
    try { const current = await veloraApi.cart(companyId!); await offlineCache.saveCart(cartCacheKey, current); setOffline(false); return current; }
    catch (error) { if (error instanceof ApiClientError && error.status === 0) { const cached = await offlineCache.readCart(cartCacheKey); if (cached) { setOffline(true); return cached; } } throw error; }
  }});
  ...
  const commit = async (next: CartResponse) => { await offlineCache.saveCart(cartCacheKey, next); client.setQueryData(["cart", user.id, companyId], next); await client.invalidateQueries({ queryKey: ["cart"] }); };
  ...
  const clear = useMutation({ mutationFn: () => veloraApi.clearCart(companyId!), onSuccess: async () => { await client.invalidateQueries({ queryKey: ["cart"] }); } });
  const value = cart.data;
  if (!companyId) return <section className="page"><Notice kind="error">Seleccione una compañía desde el catálogo antes de abrir su bolsa.</Notice></section>;
