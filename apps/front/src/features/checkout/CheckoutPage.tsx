...
export function CheckoutPage() {
  const user = useAuthStore((state) => state.user)!; const companyId = useCompanyStore((state) => state.storefrontCompanyId); const navigate = useNavigate(); const client = useQueryClient(); const [fulfillment, setFulfillment] = useState<CustomerFulfillmentType>("PICKUP"); const [warehouseId, setWarehouseId] = useState(""); const [addressId, setAddressId] = useState(""); const [notes, setNotes] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  const checkoutCacheKey = `${user.id}:${companyId ?? "none"}`;
  const data = useQuery({ queryKey: ["checkout", user.id, companyId], enabled: Boolean(companyId), queryFn: async (): Promise<CheckoutData> => {
    try {
      const [cart, warehouses, addresses] = await Promise.all([veloraApi.cart(companyId!), veloraApi.checkoutWarehouses(companyId!), veloraApi.addresses()]);
      await offlineCache.saveCart(checkoutCacheKey, cart); await offlineCache.saveCheckout(checkoutCacheKey, warehouses, addresses); return { cart, warehouses, addresses, offline: false };
    } catch (reason) {
      if (reason instanceof ApiClientError && reason.status === 0) { const [cart, checkout] = await Promise.all([offlineCache.readCart(checkoutCacheKey), offlineCache.readCheckout(checkoutCacheKey)]); if (cart && checkout) return { cart, warehouses: checkout.warehouses, addresses: checkout.addresses, offline: true }; }
      throw reason;
    }
  }});
  const preferredAddress = data.data?.addresses.find((address) => address.defaultAddress) ?? data.data?.addresses[0] ?? null;
  const modalityWarehouses = data.data?.warehouses.filter((warehouse) => fulfillment === "PICKUP" ? warehouse.pickupEligible : warehouse.deliveryEligible) ?? [];
  const effectiveWarehouseId = modalityWarehouses.some((warehouse) => warehouse.warehouseId === warehouseId) ? warehouseId : modalityWarehouses[0]?.warehouseId ?? "";
  const effectiveAddressId = addressId || preferredAddress?.id || "";
  const selectedWarehouse = modalityWarehouses.find((item) => item.warehouseId === effectiveWarehouseId) ?? null;
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (data.data.offline || !navigator.onLine) {
        ...
        await veloraApi.createOrder({ companyId: companyId!, warehouseId: effectiveWarehouseId, fulfillmentType: fulfillment, addressId: fulfillment === "DELIVERY" ? effectiveAddressId : null, notes: notes.trim() || null }, crypto.randomUUID());
  ...
  if (!companyId) return <section className="page"><Notice kind="error">Seleccione una compañía desde el catálogo antes de continuar.</Notice></section>;
