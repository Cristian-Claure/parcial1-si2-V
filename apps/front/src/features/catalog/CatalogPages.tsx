...
function ProductCard({ product }: { product: ProductResponse }) {
  ...
  const quickAdd = useMutation({
    mutationFn: (variant: VariantResponse) => veloraApi.addCartItem({ companyId: companyId!, variantId: variant.id, quantity: 1 }),
    onSuccess: () => client.invalidateQueries({ queryKey: ["cart"] }),
  });

  return <Link className="product-card" to={`/catalogo/${product.slug}`}><div className="product-image">{image ? <img src={image} alt={product.name} /> : <ProductImagePlaceholder />}{cheapest ? <button type="button" className="quick-add-button" disabled={quickAdd.isPending} onClick={(event) => { event.preventDefault(); event.stopPropagation(); if (!user) { navigate("/login", { state: { from: `/catalogo/${product.slug}` } }); return; } if (user.role === "CUSTOMER") quickAdd.mutate(cheapest); }}>{quickAdd.isPending ? "Agregando…" : "Agregar rápido"}</button> : null}</div><div><small>{product.categoryName}</small><h3>{product.name}</h3><p>{product.brand}</p><strong>{price(product).toFixed(2)} BOB</strong></div></Link>;
}
...
export function ProductDetailPage() {
  ...
  return <main className="section product-detail" key={product.id}><div className="detail-media"><div className="detail-image">{activeImage ? <img src={activeImage.imageUrl} alt={product.name} /> : <ProductImagePlaceholder />}</div>{displayImages.length > 1 ? <div className="detail-thumbnails">{displayImages.map((img, index) => <button key={img.id} type="button" className={index === activeImageIndex ? "detail-thumbnail active" : "detail-thumbnail"} onClick={() => setActiveImageIndex(index)}><img src={img.imageUrl} alt="" /></button>)}</div> : null}</div><div className="detail-copy"><span className="eyebrow">{product.categoryName}</span><h1>{product.name}</h1><p>{product.description ?? "Diseño VÉLORA seleccionado para esta colección."}</p><div className="buy-box"><div className="price-line">{selected ? `${selected.price.toFixed(2)} ${selected.currency}` : "Sin variantes"}</div><label>Variante<select value={selected?.id ?? ""} onChange={(event) => { setSelectedVariantId(event.target.value); setActiveImageIndex(0); }}>{product.variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.size} · {variant.color} · {variant.price.toFixed(2)} {variant.currency}</option>)}</select></label>{message ? <Notice kind="success">{message}</Notice> : null}<div className="actions"><button className="button primary" disabled={!selected || cartMutation.isPending} onClick={() => { if (!user) navigate("/login", { state: { from: `/catalogo/${slug}` } }); else if (user.role === "CUSTOMER" && selected) cartMutation.mutate(selected); }}>Agregar a mi bolsa</button><button className="button secondary" disabled={user?.role !== "CUSTOMER" || favoriteMutation.isPending} onClick={() => favoriteMutation.mutate()}>{favorite ? "Quitar de favoritos" : "Guardar favorito"}</button></div></div><dl className="product-meta"><div><dt>Marca</dt><dd>{product.brand}</dd></div><div><dt>Composición</dt><dd>{product.composition ?? "—"}</dd></div><div><dt>Cuidado</dt><dd>{product.careInstructions ?? "—"}</dd></div></dl></div></main>;
}
